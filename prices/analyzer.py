...
class Analyzer():
    ...
    def is_macd_crossed(self, within=1, time_unit='M', time_count=5):
        ...
        time_last_price = datetime.datetime.strptime(df.iloc[0]['datetime'], self.time_format)
    ...
    def is_candle_over_bollinger(self, toward='up', within=1, time_unit='M', time_count=5):
        table_name = 'prices_{0}{1}'.format(time_unit, time_count)
    ...
    def is_candle_closed_over_middle(self, toward='up', time_unit='M', time_count=5):
        table_name = 'prices_{0}{1}'.format(time_unit, time_count)
    ...
    def is_current_price_over_middle(self, toward='up', time_unit='M', time_count=5):
        table_name = 'prices_{0}{1}'.format(time_unit, time_count)
        # 最新のレコードを1件取得
        ...
        # 最新のレコードのdatetimeが古くないか確認
        time_now = datetime.datetime.now(datetime.timezone.utc)
        time_last_price = datetime.datetime.strptime(last_candle.iloc[0]['datetime'], self.time_format)
        time_unit_full = 'minutes'
        time_args = {
            time_unit_full: time_count*5
        }
        ...
        # APIからpriceを取得
        current_candle = self.oanda_client.get_current_candle()
        ...
        if toward == 'down':
            ...
            # borderを上回った
            if current_candle['close'] > border:
                return True

        return False

    def is_candle_keeping(self, direction='up', count=3, time_unit='M', time_count=5):
        table_name = 'prices_{0}{1}'.format(time_unit, time_count)
